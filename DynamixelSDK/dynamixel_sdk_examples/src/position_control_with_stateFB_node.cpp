...
#include "position_control_with_stateFB_node.hpp"
...
// Dynamixel ID
#define DYNA_ID 0x01 // ID of dynamixel
...
: Node("position_control_node")
{
  ...
  pos_subscriber = 
    ...
    [this](const std_msgs::msg::Int32::SharedPtr msg) -> void
    {
      ...
      uint32_t goal_position = (unsigned int)msg->data;  // Convert int32 -> uint32
  ...
  led_subscriber = 
    ...
    [this](const std_msgs::msg::Int32::SharedPtr msg) -> void
    {
      ...
      // Position Value of X series is 4 byte data.
      // For AX & MX(1.0) use 2 byte data(uint16_t) for the Position Value.
      uint8_t led = (unsigned int)msg->data;  // Convert int32 -> uint32
  ...
  vel_publisher = this->create_publisher<std_msgs::msg::Int32>("sns/vel", 10);
  temp_publisher = this->create_publisher<std_msgs::msg::Int32>("sns/temperature", 10);
  load_publisher = this->create_publisher<std_msgs::msg::Int32>("sns/load", 10);
  
  timer_ = this->create_wall_timer(1ms, std::bind(&PosCtrNode::timer_callback, this));
...
void PosCtrNode::timer_callback(){
    ...
    dxl_comm_result = packetHandler->read4ByteTxRx(
      ...
      reinterpret_cast<uint32_t *>(&sns_pos),
      &dxl_error
    );
    dxl_comm_result = packetHandler->read4ByteTxRx(
      portHandler,
      ...
      reinterpret_cast<uint32_t *>(&sns_vel),
      &dxl_error
    );
    dxl_comm_result = packetHandler->read2ByteTxRx(
      ...
      reinterpret_cast<uint16_t *>(&sns_load),
      &dxl_error
    );
    dxl_comm_result = packetHandler->read1ByteTxRx(
      ...
      reinterpret_cast<uint8_t *>(&sns_temp),
      &dxl_error
    );

    RCLCPP_INFO(
      this->get_logger(),
      "Get [ID: %d] [Position: %d] [Velocity: %d] [LOAD: %d] [TEMP: %d]",
      DYNA_ID, sns_pos, sns_vel, sns_load, sns_temp
    );

    // publish data 
    auto message = std_msgs::msg::Int32();
    message.data = sns_pos;
    pos_publisher->publish(message);

    message.data = sns_vel;
    vel_publisher->publish(message);

    message.data = sns_temp;
    temp_publisher->publish(message);

    message.data = sns_load;
    load_publisher->publish(message);
};
...
void setupDynamixel(uint8_t dxl_id)
{
  ...
  if (dxl_comm_result != COMM_SUCCESS) {
    RCLCPP_ERROR(rclcpp::get_logger("position_control_node"), "Failed to set Velocity Control Mode.");
  } else {
    RCLCPP_INFO(rclcpp::get_logger("position_control_node"), "Succeeded to set Velocity Control Mode.");
  }
...
int main(int argc, char * argv[])
{
  ...
  auto posCtrNode = std::make_shared<PosCtrNode>();
